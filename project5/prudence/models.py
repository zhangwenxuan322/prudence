...
class Control(models.Model):
    ...
    effectiveness = models.FloatField(
        choices=EFFECTIVENESS_CHOICES, default=0.0)
...
class Risk(models.Model):
    ...
    inherent_risk_rating = models.DecimalField(
    ...
    residual_probability = models.IntegerField(
        choices=PROBABILITY_CHOICES, null=True, blank=True)
    residual_risk_rating = models.DecimalField(
        max_digits=5, decimal_places=2, null=True, blank=True)
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.inherent_risk_rating:
            self.inherent_risk_rating = Decimal(
                self.inherent_probability * self.inherent_impact).quantize(Decimal('0.01'))
        ...
        if self.residual_impact is not None and self.residual_probability is not None:
            self.residual_risk_rating = Decimal(
                self.residual_impact * self.residual_probability).quantize(Decimal('0.01'))
        else:
